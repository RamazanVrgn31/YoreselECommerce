...
namespace WebAPI
{
    ...
    public partial class Startup : BusinessStartup
    {
        ...
        public override void ConfigureServices(IServiceCollection services)
        {
            ...
            {
                // 1. Consumer'ı (Tüketiciyi) Tanıtıyoruz
                // Bu satır sayesinde MassTransit, bu sınıfı takip etmesi gerektiğini anlar.
                x.AddConsumer<OrderCreatedEventConsumer>();

                // 2. RabbitMQ Bağlantı Ayarları
                x.UsingRabbitMq((context, cfg) =>
                {
                    // Ayarları appsettings.json'dan okumak daha doğrudur ama
                    // şimdilik çalışan yapıyı bozmamak için manuel giriyoruz:
                    ...
                    // 3. Kuyruk Ayarları (Endpoint)
                    // "order-created-queue" adında bir kuyruk oluşacak
                    // ve gelen mesajları 'OrderCreatedEventConsumer' işleyecek.
                    cfg.ReceiveEndpoint("order-created-queue", e =>
                    {
                        e.ConfigureConsumer<OrderCreatedEventConsumer>(context);

                        // ⭐ RETRY (YENİDEN DENEME) AYARI ⭐
                        // Hata alırsan 3 kez daha dene, her deneme arasında 5 saniye bekle.
